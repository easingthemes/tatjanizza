...
- **The first deploy on a brand-new branch name can fail on TinaCloud**, with
  `Failed to retrieve _schema.json` / `ERR_CLOUD_CHECK_FAILED`. That is the
  branch not being indexed yet, not her edit and not your code — `preflight.sh`
  cannot see it, because it never talks to TinaCloud. Someone has to click
  **Reindex** for the branch in app.tina.io. Do not change code to chase it, and
  do not tell her the edit was wrong. If an edit branch of hers is already
  indexed and green, prefer keeping her on that one.
